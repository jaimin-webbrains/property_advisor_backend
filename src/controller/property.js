...
class PropertyController {
  ...
  async addAllTsData(req, res) {
    try {
      ...
      const is_existing_details_file = await TsSchema.find({
        reraNumber: req.body.reraNumber,
        detailsFileName:
          path.resolve() + "/uploads/" + req.files.detailsFileName[0].filename,
      });
      if (is_existing_details_file.length > 0) {
        return responseHandler.errorResponse(
          res,
          400,
          `File already exist with rera no: ${req.body.reraNumber} and this modified date.`
        );
      } else {
        //checking is already exist with same reraNumber.
        const isPresentWithSameReraNumber = await TsSchema.find({
          reraNumber: req.body.reraNumber,
        }).sort({ lastModifiedDate: -1 });
        if (isPresentWithSameReraNumber.length > 0) {
          if (
            new Date(
              isPresentWithSameReraNumber[0]["_doc"]["lastModifiedDate"]
            ) > new Date(req.body.lastModifiedDate)
          ) {
            return responseHandler.errorResponse(
              res,
              400,
              `Please choose greater date than previous last modified date!.`
            );
          }
        }
        ...
        const excel_data = await propertyservice.getAllPropertyDetails(
          xlData,
          resp_data
        );
        ...
        if (
          isPresentWithSameReraNumber.length > 0 &&
          isPresentWithSameReraNumber[0].id
        ) {
          //getting property data from propertySchema by tracks_details fields which is the id of track schema index.
          const prev_proprty_data = await PropertySchema.find({
            tracks_details: isPresentWithSameReraNumber[0].id,
          });
          if (prev_proprty_data.length > 0 && prev_proprty_data[0]["_doc"]) {
            ...
            const prev_data = propertyservice.getTheNewChanges(
              res_data["_doc"],
              prev_proprty_data[0]["_doc"]
            );

            //obtaining the new data by comparing old and new propertySchema details.
            const new_data = propertyservice.getTheNewChanges(
              prev_proprty_data[0]["_doc"],
              res_data["_doc"]
            );
        ...
        if (resp_data.paId !== null) {
          try {
            const response = await PropertySchema.find({
              reraNumber: res_data.reraNumber,
            })
              .select({
                tracks_details: 1,
                _id: 0,
        ...
        if (resp_data.paId === "" || resp_data.paId === null) {
          try {
            const response = await PropertySchema.find({
              reraNumber: res_data.reraNumber,
            }).populate("tracks_details");
            await propertyservice.postDataToPropertyAdvisorForNewRera(response);
          } catch (error) {
            return responseHandler.errorResponse(res, 500, error.message);
          }
        }
        return responseHandler.successResponse(
          res,
          201,
          "TS data added successfully.",
          { track_data: resp_data, property_data: res_data }
        );
      }
    } catch (error) {
  ...
  async getReraDetailsByNumber(req, res) {
    try {
      ...
      if (reraNumber !== "" && reraNumber !== undefined) {
        const response = await PropertySchema.find({ reraNumber: reraNumber })
          .select({
            tracks_details: 1,
            "Promoter Information - Organization.Name": 1,
            "Project address details.District": 1,
  ...
  async getPropertyDetailsByReraNumber(req, res) {
    try {
      ...
      const histories = await propertyFieldHistorySchema.find({
        reraNumber: reraNumber,
      });
  ...
  async bulkAddProperties(req, res) {
    ...
    try {
      const workbook = XLSX.readFile(
        path.resolve() + "/uploads/" + req.files.bulkfile[0].filename
      );
      ...
      //converting sheet to json.
      const project_xlData = XLSX.utils.sheet_to_json(
        workbook.Sheets[sheet_name_list[0]],
        { defval: "" }
      );
        ...
        for (let row in project_xlData) {
          if (filePerUpload.indexOf(parseInt(row)) > -1) {
            let percentage = Math.ceil(
              (row / (project_xlData.length - 1)) * 100
            );
            if (percentage <= 99) {
              parseInt(row) === 0
                ? io.emit("get", 1)
          ...
          try {
            const obtained_tracks_data =
              propertyservice.convertToTrackDataFromExcel(project_xlData[row]);
            //checking if data already present with same reraNumber and lastModifiedDate.
            const is_existing_details_file = await TsSchema.find({
              reraNumber: obtained_tracks_data.reraNumber,
              lastModifiedDate: new Date(obtained_tracks_data.lastModifiedDate),
            });
            if (is_existing_details_file.length > 0) {
              cancelledArray.push({
              ...
              if (isPresentWithSameReraNumber.length > 0) {
                if (
                  new Date(
                    isPresentWithSameReraNumber[0]["_doc"]["lastModifiedDate"]
                  ) > new Date(obtained_tracks_data.lastModifiedDate)
                ) {
                  cancelledArray.push({
                    reraNumber: project_xlData[row]["RERA No"],
              ...
              const workbook = XLSX.readFile(
                path.resolve() +
                  "/uploads/" +
                  project_xlData[row]["Details File Name"] +
                  ".xlsx"
              );
              ...
              let excel_data;
              try {
                excel_data = await propertyservice.getAllPropertyDetails(
                  xlData,
                  resp_data
                );
              } catch (error) {
                await TsSchema.deleteOne({
                  reraNumber: project_xlData[row]["RERA No"],
                });
                cancelledArray.push({
                  reraNumber: project_xlData[row]["RERA No"],
              ...
              if (
                ...
                if (
                  ...
                  //deleting the most recently property data ,just before this current data.
                  await PropertySchema.deleteOne({
                    _id: prev_proprty_data[0].id,
                  });
                }
              }
              if (resp_data.paId !== null) {
                try {
                  await propertyservice.postDataToPropertyAdvisor(
                    res_data,
                    resp_data
                  );
                } catch (error) {
        ...
        try {
          ...
          fs.writeFileSync(
            path.resolve() +
              ...
              d.getFullYear() +
              ".txt",
            JSON.stringify(cancelledArray, null, 2)
          );
